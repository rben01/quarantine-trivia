...
QUESTIONS_CSV_FILE = next(Path(".").glob("*.csv"))
TITLE = "Welcome to Saturday Night Trivia XVII!"
...
SECTION_ORDER = [
    ...
    "Number One Songs",
    "Things Dean Martin Never Said",
    "Hey, look what I found(ed)!",
...
def get_trivia_items() -> List[TriviaItem]:
    ...
    df = df[df[Q_COL].notna() & df[TOPIC_COL].isin(SECTION_ORDER)]

    for c in [Q_COL, A_COL, SECTION_COL, TOPIC_COL]:
        ...
        df[c] = (
            ...
            .str.replace("’", "'", regex=False)
            .str.replace("{\\''", '{\\"', regex=False)
            # .str.replace(r"(?:(?!\\))_", r"\textunderscore{}", regex=True)
            .str.replace("&", r"\&", regex=False)
            .str.replace("%", r"\%", regex=False)
            .str.replace("#", r"\#", regex=False)
            ...
            .str.replace(r"(\d+)([a-z]{2,})", r"\1\\textsuperscript{\2}", regex=True)
        )
        if c == A_COL:
            df[c] = (
                df[c]
    ...
    regular_df: pd.DataFrame = df[df[SECTION_COL] != "Bonus"].copy()
    bonus_df: pd.DataFrame = df[df[SECTION_COL] == "Bonus"].copy()
    ...
    for section, group in g:
        ...
        for i in range(n_subsections):
            ...
            for _, row in this_round_df.iterrows():
                image_files: Dict[str, Optional[Union[Path, str]]] = {}
...
def make_latex(trivia_items: List[TriviaItem], include_images: bool = True) -> str:
    ...
    def append_qs_and_as(trivia_items: List[TriviaItem]):
        ...
        latex_items.append(BeamerFrame(r"\subsection{Answers}"))
    ...
    preamble = BeamerFrame(preamble.replace(r"{{DRAFT_}", _draft_str))
    preamble = BeamerFrame(preamble.replace(r"{{TITLE_}", TITLE))
